export default async function handler(req, res) {
  ...
  try {
    ...
    const systemPrompt = `You are Zola, an AI-powered local discovery guide. You ONLY respond with valid JSON — no markdown, no explanation, just raw JSON.
  ...
  "queries": [
    {
      ...
      "time": "9:00 AM",
      "type": "Coffee Shop",
      "zola_pick_reason": "one sentence why this type of place fits the vibe"
    }
  ],
  "zola_note": "A warm, personal closing note from Zola — like advice from a local friend. 2-3 sentences. Conversational."
}`;
    ...
    for (const item of plan.queries) {
      try {
        ...
        stops.push({
          ...
          type: item.type,
          description: place.editorial_summary?.overview || item.zola_pick_reason || '',
