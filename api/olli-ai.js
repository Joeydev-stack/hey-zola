...
⏺ export default async function handler(req, res) {
  ...
  - Queries should be specific enough to find great matches (e.g. "cozy indie coffee shop Austin TX" not just "coffee 
  Austin")
  ...
  `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(city)}&key=${PLACES_KEY}`;
        ...
        if (geoData.results && geoData.results.length > 0) {
          cityLat = geoData.results[0].geometry.location.lat;
          cityLng = geoData.results[0].geometry.location.lng;
        }
      } catch (e) {
